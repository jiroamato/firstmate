...
new_home() {
  # mktemp rather than an incremented counter: every caller reads this through a
  # command substitution, so a counter would be incremented in the subshell and
  # lost, handing every test the same home and every whole-fleet run every other
  # test's clones.
  local h
