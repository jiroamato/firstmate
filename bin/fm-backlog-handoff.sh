...
with_remote_route_locks() { # <secondmate-id> <function> <args...>
  ...
  fm_lock_acquire_wait "$ACTIVE_REGISTRY_LOCK" || { ACTIVE_REGISTRY_LOCK=''; return 1; }
  ...
  fm_lock_acquire_wait "$ACTIVE_HANDOFF_LOCK" || { ACTIVE_HANDOFF_LOCK=''; release_remote_locks; return 1; }
...
if [ "$REMOTE" = 1 ]; then
  ACTIVE_HANDOFF_LOCK="$STATE/.backlog-handoff-$ID.lock"
  fm_lock_acquire_wait "$ACTIVE_HANDOFF_LOCK" || { ACTIVE_HANDOFF_LOCK=''; release_remote_locks; exit 1; }
