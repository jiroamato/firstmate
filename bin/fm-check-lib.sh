...
fm_custom_check_snapshot_prepare() {
  ...
  # Capability-aware private-file validation (regular non-symlink file, 600 or
  # the probed owner-match substitution, same device, link count 1) - a raw
  # mode compare here would reject every snapshot on a mode-incapable host.
